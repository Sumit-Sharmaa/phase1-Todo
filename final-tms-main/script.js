...
function addTask() {
    if(validateTaskName() && validateDate() && validateStatus()){
...
function searchTasks(){
    ...
    if(searchType == 'By-Name'){
        const filterdTasks = tasks.find(task => task.name === searchValue);
        console.log(filterdTasks)
        filterdTasks.style.display='none'
    }
    else{
        const filterdTasks = tasks.find(task => task.name === searchValue);
        filterdTasks.style.display='block';
    }
    
}
const results = tasks.filter(searchTasks);
